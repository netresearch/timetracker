...
A tag push releases. Two workflows run on it. `docker-publish.yml` builds and signs the images: every pushed tag carries a Cosign keyless signature, buildx SLSA provenance (`mode=max`), an in-image SBOM and a merged CycloneDX SBOM attestation. `release.yml` calls `netresearch/.github`'s `release-source-archive.yml`, which checks out the tag, refuses a lightweight tag, builds `timetracker-vX.Y.Z-source.tar.gz` with `git archive --format=tar | gzip -9 -n`, generates SPDX and CycloneDX SBOMs, attests build provenance and the SBOM, writes `checksums.txt`, signs every file with Cosign and creates the GitHub Release last, in one call, with all files attached. The build runs inside that reusable, which this repository cannot edit, so the provenance is SLSA Build Level 3 (GitHub: attestations alone give Level 2, a reusable workflow isolating the build gives Level 3). Do not create the release by hand: `release.yml` would then find a release it did not build, and an immutable release refuses the uploads. `reproducible-archive.yml` runs after each Release run and weekly, rebuilds the archive and compares it with `checksums.txt`. Verification needs `--signer-workflow netresearch/.github/.github/workflows/release-source-archive.yml`; SECURITY.md carries the commands and the different layout of v6.4.0. Order: verify main CI green → `git tag -s vX.Y.Z -m "vX.Y.Z"` → `git push origin vX.Y.Z` → the release appears with auto-generated notes → rewrite the notes with `gh release edit vX.Y.Z --notes-file <notes>` (only the notes; files and tag are fixed). The metadata-action strips the leading `v`: images are `:6.0.0`/`:6.0`/`:6`. Credit contributors inline per change with `@mentions` (GitHub builds the Contributors row from body mentions); auto-generated notes miss direct pushes — cross-check via the compare API. Credit the human driving an agent-authored commit (committer / `Co-authored-by`), never a bot author.
