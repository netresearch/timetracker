...
- deleted in Jira → move detection first (a matching new worklog re-links the entry); otherwise a clean entry is deleted, a locally modified entry is parked as **orphaned**. This needs a complete remote read: when an issue's worklogs could not be fetched, a worklog could not be read, or the issue search was capped (each reported as an `error`/`truncated` item), a missing worklog may still exist, so the entry is neither deleted nor parked but counted as `absence_unverified` and re-checked by the next run,
