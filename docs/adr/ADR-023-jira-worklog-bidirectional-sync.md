...
**Status:** Accepted — fully implemented (Phases 1–4).
**Date:** 2026-07-09
...
Implementation note (Phase 4b): the SolidJS UI ships the self-service import (Settings), the admin sync area (run history, triggers, ticket-system sync config), and the conflict-resolution screen, all against the v2 endpoints; a list-runs endpoint was added to back the run history.
...
- The window a run reads is TT's, not Jira's: a worklog re-dated outside it drops out of the search while the entry, dated by `entries.day`, stays a candidate. Its absence from the read is therefore never evidence on its own — `RemoteWorklogReader::readOne()` reads the worklog from the issue its entry names before anything is concluded, so a re-dated worklog is pulled rather than treated as deleted. That read also runs before move detection: otherwise a separate booking sharing the entry's *old* start and duration could be relinked onto it while its real worklog is still there. A failed read is a per-worklog gap (`absence_unverified`), never a deletion.
- Interaction with the internal-ticket-system mirror (`internalJiraTicketOriginalKey`): a mirrored entry is booked into the **internal** Jira, while its project still points at the external ticket system. Sync therefore never treats such an entry as a candidate (`EntryRepository::findJiraSyncCandidates` excludes a set `internalJiraTicketOriginalKey`), so it is neither pushed nor reconciled — the intended behaviour. What is **not** covered is the reverse direction: the linked-worklog lookups (`findOneByWorklogIdAndTicketSystem`, `findByWorklogIdsAndTicketSystem`) are scoped by `project.ticketSystem` too, so a run against the internal ticket system does not recognise those worklogs as belonging to an entry and reports them as `remote_only` import candidates. Closing it properly means recording which ticket system a worklog id belongs to, rather than deriving it from the project — a schema change, deliberately not made here. Operational rule until then in [worklog-sync.md](../worklog-sync.md#internal-jira-mirror): leave `sync_default_activity_id` unset on an internal mirror ticket system.
...
4. ~~Internal-ticket-system mirror interaction~~ **Resolved (§7), partly as a documented limitation:** mirrored entries are excluded from sync candidates, so nothing is pushed or reconciled against the wrong Jira; a run against the internal ticket system still reports their worklogs as import candidates, which stays open until a worklog carries its owning ticket system.
