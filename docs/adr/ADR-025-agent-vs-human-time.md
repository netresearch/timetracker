...
- **`estimated` / `touchpoints` are honoured only in the API-token channel, for either source.** The agent sets them on its walltime entry and on the delegated `source=human` estimate it writes alongside (§3). A session request never sets them; confirming a delegated estimate in the web UI clears both.
