...
- The two entries of one dual-write are linked symmetrically (`entries.paired_entry_id`, unique, `ON DELETE SET NULL`). Deleting either half through the worklog UI or the `delete_entry` tool deletes both. A delete driven by the ticket-system sync (the remote worklog of one half is gone) removes only that half and the survivor loses its link, because the remote change is a fact about that half alone.
