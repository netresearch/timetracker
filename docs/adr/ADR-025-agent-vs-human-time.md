...
- **`source=agent` is only honoured in the API-token channel** (an ADR-021 PAT — the agent's channel). A session-authenticated request (the web UI) is **always** `source=human`, regardless of what the body says. Otherwise a person could self-mark manual work as `agent` and have it silently dropped from attendance/ArbZG (§5) — a way to hide worked hours.
  - **Exception — correcting an existing agent entry:** a session save of an entry that is *already* `source=agent` keeps its attribution (source, estimated, touchpoints, responsible user) and changes only the edited fields, so a correction does not silently turn machine time into human labour (§3 allows correcting agent-logged entries). The trade-off is accepted: such an edit may also move the entry's day or times while it stays agent time. The body still cannot *create* agent attribution in a session.
...
- **`estimated` / `touchpoints` are honoured only in the API-token channel, for either source.** The agent sets them on its walltime entry and on the delegated `source=human` estimate it writes alongside (§3). A token update that omits `source`, `estimated` or `touchpoints` keeps the stored values rather than resetting the entry to a plain human self-log. A session request never sets them; confirming a delegated estimate in the web UI clears both.
...
- The two entries of one dual-write are linked symmetrically (`entries.paired_entry_id`, unique, `ON DELETE SET NULL`). Deleting either half through the worklog UI or the `delete_entry` tool deletes both. A delete driven by the ticket-system sync (the remote worklog of one half is gone) removes only that half and the survivor loses its link, because the remote change is a fact about that half alone. Editing either half keeps its source.
