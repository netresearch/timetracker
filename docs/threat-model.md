...
**Residual risk** — the policy is **report-only** for its first release. It reports what it would have blocked and blocks nothing; `e2e/csp.spec.ts` is what reads those reports, since there is no `report-uri` collector. Until the header is switched to enforcing, escaping remains the only layer that actually stops execution. `style-src` still carries `'unsafe-inline'` for the inline `<style>` on the error pages. Assets nginx serves straight off disk bypass PHP and carry no headers at all. Tracked in [#739](https://github.com/netresearch/timetracker/issues/739).
