...
**Controls, second layer** — `SecurityHeadersSubscriber` sets `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff` and `Referrer-Policy: same-origin` on every main response, without overwriting a value a reverse proxy already set.
**Controls, third layer** — a nonce-based Content Security Policy: `default-src 'self'`, `script-src` naming a per-request nonce and nothing else, `frame-src` limited to the https origin of the corporate-navigation iframe, and `object-src 'none'`. All nine inline `<script>` blocks in `templates/` carry that nonce — the eight executable ones and the `application/ld+json` block — so injected markup has none and is refused. It is set on main-request HTML responses that do not already carry a policy: a sub-request renders a fragment whose headers are discarded, and a policy a reverse proxy already set is left alone, because the browser intersects two policies and a violation then cannot be attributed.
**Residual risk** — the policy is enforced, so injected inline script is refused rather than reported, but its coverage is the pages `e2e/csp.spec.ts` drives: login, the shell, the worklog, the evaluation, the admin area and the three settings sections. A page none of those reach could still carry an un-nonced inline `<script>`, and it would now be broken rather than merely reported. `style-src` keeps `'unsafe-inline'` in debug builds only, where the Vite dev server injects a `<style>` nothing can nonce; the CI end-to-end run exercises that debug arm, so the production arm is covered by the builder's unit tests rather than by a browser. Files nginx serves straight off disk bypass PHP, so the three static headers are set in `docker/nginx/*.conf` at server level instead; they carry no Content-Security-Policy, which is nonce-based and can only be minted by the application. Tracked in [#739](https://github.com/netresearch/timetracker/issues/739).
