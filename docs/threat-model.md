...
**Residual risk** — **there is still no Content Security Policy.** Against injected markup that executes, escaping remains the only layer; the three headers above answer framing, MIME sniffing and referrer leakage, not script execution. A policy needs nonces on eight inline `<script>` blocks and an allowance for the `#nrnavi` corporate-navigation iframe, so it follows separately. Assets nginx serves straight off disk bypass PHP and carry no headers at all. Both tracked in [#739](https://github.com/netresearch/timetracker/issues/739).
