...
**Controls, third layer** — a nonce-based Content Security Policy on every HTML response: `default-src 'self'`, `script-src` naming a per-request nonce and nothing else, `frame-src` limited to the origin of the corporate-navigation iframe, and `object-src 'none'`. Each of the eight inline `<script>` blocks carries that nonce, so injected markup has none and is refused.
