...
| GitHub Actions | Third-party actions pinned to a full commit SHA with the version in a trailing comment. Two deliberate exceptions: reusable workflows from `netresearch/.github` use `@main` by convention, being inside our own trust boundary, and `slsa-framework/slsa-github-generator` is referenced by tag because it refuses to run from a commit SHA. |
