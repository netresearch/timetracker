...
| GitHub Actions | Third-party actions pinned to a full commit SHA with the version in a trailing comment. One deliberate exception: reusable workflows from `netresearch/.github` use `@main` by convention, being inside our own trust boundary. `slsa-framework/slsa-github-generator` used to be a second one; it is no longer referenced at all, because its own workflow calls four nested actions by tag and `sha_pinning_required` rejects the run at the first of them. Provenance comes from `actions/attest-build-provenance`, which is SHA-pinned like everything else. |
