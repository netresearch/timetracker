...
- Page title: `document.title` stays `settings_title` (owned by the Layout effect); the active section is conveyed by the nav's `aria-current` and each section's fieldset legend. (Per-section document titles would race the Layout's title effect — not worth it.)
