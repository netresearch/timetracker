...
export function syncNav(pathname: string): void {
  ...
  // Main-bar links, the relocated Settings/Help icon actions and the mobile
  // drawer links (but not the worktime badges, which also carry
  // data-nav="month").
  for (const link of document.querySelectorAll<HTMLAnchorElement>('.main-nav-link[data-nav], .header-icon-link[data-nav], .drawer-link[data-nav]')) {
