...
final class Version20250901_AddPerformanceIndexes extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_user_day ON entries (user_id, day DESC)');

        // Index for date range queries
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_day ON entries (day)');

        // Index for customer queries
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_customer ON entries (customer_id)');

        // Index for project queries
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_project ON entries (project_id)');

        // Index for activity queries
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_activity ON entries (activity_id)');

        // Index for ticket queries
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_ticket ON entries (ticket)');
        ...
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_user_sync ON entries (user_id, synced_to_ticketsystem)');

        // Index for worklog_id lookups
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_worklog ON entries (worklog_id)');

        // Composite index for date + start time sorting
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_entries_day_start ON entries (day DESC, start DESC)');
    }
