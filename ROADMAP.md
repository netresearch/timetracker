...
- **Security response headers** ([#739](https://github.com/netresearch/timetracker/issues/739)). The application sets none: no `Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options` or `Referrer-Policy`, in neither the nginx configuration nor a Symfony listener. Twig autoescaping and the SPA's own escaping are currently the only layer against injected markup. Adding a policy needs the SPA's inline bootstrap accounted for, so it is a change with a test surface rather than a one-line header.
