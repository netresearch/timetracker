...
- **Widen what the CSP is proven against** ([#739](https://github.com/netresearch/timetracker/issues/739)). The policy is enforced and `style-src` is nonce-based outside debug builds. What is not covered is a page `e2e/csp.spec.ts` does not drive, and the production `style-src` — the end-to-end stack runs with `APP_DEBUG=1`, so the browser never evaluates the stricter arm. Either add the surfaces to the spec, or run one end-to-end shard without debug.
