...
- **Enforce the Content Security Policy** ([#739](https://github.com/netresearch/timetracker/issues/739)). The nonce-based policy ships report-only: it reports what it would block and blocks nothing, and `e2e/csp.spec.ts` reads those reports. Switching the header from `Content-Security-Policy-Report-Only` to `Content-Security-Policy` is a one-word change once the suite has been green across a release. Narrowing `style-src` off `'unsafe-inline'` is the other half.
